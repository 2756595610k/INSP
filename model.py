...
class MemoryEncoder(nn.Module):
    ...
    def forward(self, input, interval, mask, hidden=None):
        if hidden == None:
            h0 = th.zeros(input.size(0), self.hidden_size)
            c0 = th.zeros(input.size(0), self.hidden_size)
            if th.cuda.is_available():
                h0, c0 = h0.cuda(), c0.cuda()
            hidden = (h0, c0)
            h0_ = th.zeros(input.size(0), self.hidden_size)
            c0_ = th.zeros(input.size(0), self.hidden_size)
            if th.cuda.is_available():
                h0_, c0_ = h0_.cuda(), c0_.cuda()

        hs = th.zeros(input.size(0), input.size(1), self.hidden_size)
        cs = th.zeros(input.size(0), input.size(1), self.hidden_size)
        if th.cuda.is_available():
            hs, cs = hs.cuda(), cs.cuda()

        for i in range(input.size(1)):
            #hs[:, i, :],cs[:, i, :] =self.lstm_cell(x[:, i, :], hidden)
            hs[:, i, :], hidden = self.lstm_cell(input[:, i, :], interval[:, i], hidden)
            cs[:, i, :] = hidden[1]
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_context, last_hidden, encoder_matrix, neighbor_matrix):
        ...
        output = self.net(rnn_output)
        #output = self.net(concat_input)
        Q = context.data.cpu().numpy()  # tensor转换成array
        Win=neighbor_context.data.cpu().numpy()
        TT=output.data.cpu().numpy()
        # print("Q:", Q)
        np.save("analysis/time_attn.npy",Q, True)
        np.save("analysis/social_attn.npy",Win, True)
        np.save("analysis/output.npy", TT)
...
def train_batch(input_batches, input_lengths, input_interval, input_mask, target_batches, target_mask,
    ...
    neighbor_encoder_optimizer.zero_grad()
    decoder_optimizer.zero_grad()
    batch_size = input_batches.size(0)
    # neighbor_input:(256,8,125,6)  tmp_neighbor_input:(2048,125,6)
    ...
    # neighbor_length(256,8,125)
    tmp_neighbor_length = neighbor_length.view(neighbor_length.size(0) * neighbor_length.size(1), neighbor_length.size(2))

    # Run words through encoder
    # encoder_outputs(256,125,32)  encoder_hidden[(256,125,32),(256,125,32)]   encoder_context: (256,32,32)
    encoder_outputs, encoder_hidden, encoder_context = encoder(input_batches, input_interval, input_mask, None)
    # neighbor_encoder_outputs(2048,125,32)  neighbor_encoder_hidden[(2048,125,32),(2048,125,32)]  neighbor_encoder_context(256,8,32,32)
...
def predict_batch(input_batches, input_lengths, input_interval, input_mask, target_batches, test_mask,
               neighbor_input, neighbor_interval, neighbor_length,
